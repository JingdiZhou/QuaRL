...
class ExperimentManager:
    ...
    def __init__(
            ...
            save_freq: int = -1,  # -1 means no checkpoint
    ...
    def setup_experiment(self) -> tuple[BaseAlgorithm, dict[str, Any], VecEnv] | None:
        ...
        """
        hyperparams, saved_hyperparams = self.read_hyperparameters()  # load the parameters from hyperparams/xxx.yml
        hyperparams, self.env_wrapper, self.callbacks, self.vec_env_wrapper = self._preprocess_hyperparams(
            hyperparams)  # process the hyperparams from loading
        ...
        if self.continue_training:
            ...
            model = ALGOS[self.algo](
                ...
                device=self.device,
                optimize_choice=self.optimize_choice,
                **self._hyperparams,
            )

        self._save_config(saved_hyperparams)
        return model, saved_hyperparams, env

    def learn(self, model: BaseAlgorithm, env) -> tuple[Any, Any]:
        """
        :param model: an initialized RL model
        """
        global std_reward, mean_reward
        kwargs = {}
    ...
    def read_hyperparameters(self) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        print(f"Loading hyperparameters from: {self.config}")  # hyperparams/{algo}.yml
    ...
    def _preprocess_hyperparams(  # noqa: C901
        ...
        if self.n_timesteps > 0:
            ...
            self.n_timesteps = int(hyperparams["n_timesteps"])  # get n_timesteps from hyperparams(xxx.yml
