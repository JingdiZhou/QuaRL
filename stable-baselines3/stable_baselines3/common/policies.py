...
from micronet.compression.quantization.wqaq.iao.quantize import QuantLinear
...
class BasePolicy(BaseModel, ABC):
    ...
    :param quantized: Quantization bits of the model
...
class ActorCriticPolicy(BasePolicy):
    ...
    :param quantization bit of the model
    ...
    def __init__(
        ...
        '''QAT'''
        # QAT of self.features_extractor, which is FlattenExtractor
        self.features_extractor = self.make_features_extractor()  # FlattenExtractor
        if self.q != 32:
            self.features_extractor = quant_iao.prepare(
        ...
        if self.share_features_extractor:
            ...
            self.pi_features_extractor = self.features_extractor
            self.vf_features_extractor = self.make_features_extractor()  # FlattenExtractor
    ...
    def _build_mlp_extractor(self) -> None:
        ...
        if self.q != 32:
            self.mlp_extractor = quant_iao.prepare(
                    self.mlp_extractor,
                    inplace=False,
                    a_bits=self.q,
                    w_bits=self.q,
            ...
            self.mlp_extractor.cuda()
        # self.mlp_extractor = th.nn.DataParallel(
        #     self.mlp_extractor, device_ids=range(th.cuda.device_count())
        # )
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        """
        self._build_mlp_extractor()   #build mlp_extractor network，we create the quantization instance in it
        ...
        if isinstance(self.action_dist, DiagGaussianDistribution):
            ...
            self.action_net = self.action_dist.proba_distribution_net(self.q, latent_dim=latent_dim_pi)
        else:
            raise NotImplementedError(f"Unsupported distribution '{self.action_dist}'.")

        self.value_net = nn.Linear(self.mlp_extractor.latent_dim_vf, 1)

        # self.value_net = QuantLinear(
        #                     latent_dim,
        #                     self.action_dim,bias=False,
        #                     a_bits=self.q,
        #                     w_bits=self.q,
        #                     q_type=q_type,
        #                     q_level=q_level,
        ...
        # base_optimizer=th.optim.SGD
        # self.optimizer = SAM(self.parameters(), base_optimizer, lr=lr_schedule(1))
